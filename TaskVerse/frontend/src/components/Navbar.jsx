...
const Navbar = () => {
    ...
    return (
        <div className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-8 px-8 pointer-events-none">
            <nav className="rounded-[4rem] px-24 py-10 pointer-events-auto flex items-center justify-between w-full max-w-[2000px] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.4)] border border-white/40 dark:border-white/10 bg-white/10 dark:bg-black/30 backdrop-blur-[60px] transition-all duration-1000 ease-in-out">
                <div className="flex items-center ml-4">
                    <span className="hidden lg:block text-4xl font-black font-outfit tracking-tighter text-gradient leading-none">TaskVerse</span>
                </div>

                <div className="flex items-center gap-6 p-2 rounded-[2.5rem]">
                ...
                <div className="flex items-center space-x-6">
                    ...
                    <button
                        onClick={logout}
                        className="w-12 h-12 rounded-[1.25rem] flex items-center justify-center text-[#FF453A] hover:bg-red-500/10 transition-all active:scale-90"
                        title="Logout"
                    >
